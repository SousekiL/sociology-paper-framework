...
`library/` contains lawful public **metadata and available abstracts**, never full papers. Its update process is append-only and DOI-deduplicated: new scans enrich the archive and never overwrite accumulated records. Each week it selects one explicitly licensed open-access PDF, covers the complete extracted text in chunks, and stores only structured design notes—not PDFs or extracted article text. Automated reading accepts an OpenAI Chat Completions-compatible API: configure `LLM_API_KEY` as a repository secret and optionally `LLM_BASE_URL` and `LLM_MODEL` as repository variables; official OpenAI remains the default.
