...
`library/` 保存的是文章的公开**元数据与公开摘要**，不是论文全文。更新脚本遵循追加与 DOI 去重规则：旧记录永不被新扫描替换；新记录只会丰富同一资料库。每周还会精选最多三篇明确开放获取的 PDF，覆盖式分段读取全文后，只保存结构化研究设计卡（研究问题、理论机制、数据、测量、模型、诊断与局限），不提交 PDF 或原文文本。
